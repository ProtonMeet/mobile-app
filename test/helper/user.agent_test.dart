...
void main() {
  group('UserAgent Tests', () {
    ...
    setUp(() {
      ...
      when(mockPackageInfo.appName).thenReturn('Proton Meet');
    ...
    test('User agent string on Linux', () async {
      ...
      when(
        mockDeviceInfo.linuxInfo,
      ).thenAnswer((_) async => Future.value(mockLinuxDeviceInfo));
      ...
      expect(ua, 'ProtonMeet/1.0.0 (Ubuntu 20.04; Ubuntu 20.04 LTS)');
      final appVersion = await userAgent.appVersion;
      expect(appVersion, "linux-meet@1.0.0.33-dev");
      final display = await userAgent.display;
      expect(display, "ProtonMeet 1.0.0-dev (33)");
    });
    ...
    test('User agent string on macOS', () async {
      ...
      when(
        mockDeviceInfo.macOsInfo,
      ).thenAnswer((_) async => Future.value(mockMacOsInfo));
      ...
      final ua = await userAgent.ua;
      expect(ua, 'ProtonMeet/1.0.0 (macOS 10.15.7; MacBookPro16,1)');
      final appVersion = await userAgent.appVersion;
      expect(appVersion, "macos-meet@1.0.0.33-dev");
      final display = await userAgent.display;
      expect(display, "ProtonMeet 1.0.0-dev (33)");
    });
    ...
    test('User agent string on iOS', () async {
      ...
      ).thenAnswer((value) async => Future.value(mockIosInfo));
      final userAgent = UserAgent(
        deviceInfo: mockDeviceInfo,
        packageInfo: Future.value(mockPackageInfo),
      );
      ...
      expect(ua, 'ProtonMeet/1.0.0 (iOS 14.0; iPhone8,1)');
      final appVersion = await userAgent.appVersion;
      expect(appVersion, "ios-meet@1.0.0.33-dev");
      final display = await userAgent.display;
      expect(display, "ProtonMeet 1.0.0-dev (33)");
    });
    ...
    test('User agent string on Android', () async {
      ...
      ).thenAnswer((_) async => Future.value(mockAndroidInfo));
      ...
      final ua = await userAgent.ua;
      expect(ua, 'ProtonMeet/1.0.0 (Android 11; Google Pixel 4)');
      final appVersion = await userAgent.appVersion;
      expect(appVersion, "android-meet@1.0.0.33-dev");
      final display = await userAgent.display;
      expect(display, "ProtonMeet 1.0.0-dev (33)");
    });
    ...
    test('User agent string on windows', () async {
      ...
      ).thenAnswer((_) async => Future.value(mockWindowsDeviceInfo));
      ...
      final ua = await userAgent.ua;
      expect(ua, 'ProtonMeet/1.0.0 (Windows 5; 21H2)');
      final appVersion = await userAgent.appVersion;
      expect(appVersion, "windows-meet@1.0.0.33-dev");
      final display = await userAgent.display;
      expect(display, "ProtonMeet 1.0.0-dev (33)");
    });
  });
