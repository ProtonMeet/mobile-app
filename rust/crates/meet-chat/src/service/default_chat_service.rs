...
#[derive(Debug, Clone)]
pub struct DefaultChatService<P: KeyProvider> {
    key_provider: P,
    history: Vec<ChatHistoryEntry>,
}

impl<P: KeyProvider> DefaultChatService<P> {
...
impl<P: KeyProvider> ChatService for DefaultChatService<P> {
    fn key_provider(&mut self) -> &mut dyn KeyProvider {
...
mod tests {
    ...
    fn test_basic_chat_service_new() {
        ...
        let mut service = DefaultChatService::new(key_provider);
    ...
    fn test_encrypt_decrypt_round_trip() {
        let key_provider = create_test_key_provider();
        let service = DefaultChatService::new(key_provider);
    ...
    fn test_add_encrypted_message() {
        ...
        let mut service2 = DefaultChatService::new(create_test_key_provider());
