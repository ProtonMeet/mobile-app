...
import 'package:meet/views/components/page_route.dart';
import 'package:meet/views/scenes/core/bloc.view.dart';
import 'package:meet/views/scenes/core/responsive.dart';
import 'package:meet/views/scenes/core/view.dart';
import 'package:meet/views/scenes/core/view.navigator.dart';
...
abstract class Coordinator implements ViewNavigator {
  ...
  /// When true, unknown routes should be ignored (no-op) instead of showing
  /// a "page not found" dialog/screen.
  ///
  /// This is useful for sensitive flows like an active meeting room where
  /// accidental/unknown navigations (e.g. deep link handling) should not
  /// interrupt the user.
  static bool suppressUnknownRouteUi = false;
  ...
  void pushReplacement(ViewBase view, {bool fullscreenDialog = false}) {
    Future.delayed(Duration.zero, () {
      Coordinator.rootNavigatorKey.currentState?.pushReplacement(
        MaterialPageRoute(
          settings: RouteSettings(name: view.getRouteName()),
          builder: (context) {
            return view;
          },
  ...
  void pushReplacementNew(BlocView view, {bool fullscreenDialog = false}) {
    Future.delayed(Duration.zero, () {
      Coordinator.rootNavigatorKey.currentState?.pushReplacement(
        MaterialPageRoute(
          settings: RouteSettings(name: view.getRouteName()),
