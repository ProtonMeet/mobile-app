...
class CargoBuildOptions {
  ...
  final bool useLocked;

  CargoBuildOptions({
    required this.toolchain,
    required this.flags,
    required this.customVersion,
    required this.useLocked,
  });
  ...
  static CargoBuildOptions parse(YamlNode node) {
    ...
    List<String> flags = [];
    bool useLocked = true; // default to --locked
    for (final MapEntry(:key, :value) in node.nodes.entries) {
      ...
      } else if (key case YamlScalar(value: 'use_locked')) {
        if (value case YamlScalar(value: bool value)) {
          useLocked = value;
          continue;
        }
        throw SourceSpanException(
            'Invalid value for "use_locked". Must be a boolean.',
            value.span);
      } else {
        throw SourceSpanException(
            'Unknown cargo option type. Must be "toolchain", "extra_flags", or "use_locked".',
            key.span);
      }
    }
    _log.info('Parsed cargo options: $toolchain, $flags, $customVersion, useLocked: $useLocked');
    return CargoBuildOptions(
        toolchain: toolchain, flags: flags, customVersion: customVersion, useLocked: useLocked);
  }
}
